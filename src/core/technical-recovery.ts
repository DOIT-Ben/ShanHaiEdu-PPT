...
export function technicalFailureDisposition(errorCode: string): TechnicalFailureDisposition | null {
  ...
  return /TIMEOUT|RATE_LIMIT|429|408|425|5\d\d|UNAVAILABLE|TEMPORARY|GATEWAY|NETWORK|UNKNOWN|NO_HEALTHY_ROUTE|VISUAL_REVIEW_FAILED|PAGE_REVIEW_FAILED|DECK_REVIEW_FAILED|DELIVERY_FAILED/.test(normalized)
    ? 'RETRYABLE'
    : null
}

...
- [x] 接入 Wikimedia Commons 与 Openverse，限制许可白名单并保存来源清单
- [x] 增加检索相关性、儿童安全、SSRF、MIME、尺寸和重定向防护
- [x] 检索命中时跳过 AI 生图和预算占用，失败时保留 AI 兜底
- [x] 归一化网络图片并在 PPTX 中保留 JPEG，控制交付体积
- [x] 增加失败交付专用重试动作并保持原幂等键
- [x] 使用真实教材完成 8 页分层课件隔离交付和结构验收

...
1. 冻结 `LAYERED_COURSEWARE_V3` Run 选项、独立封面策略、分层页面和元素合同。
2. 扩展规划输入与 Mock Planner，生成知识绑定的底图、多素材、文字和原生形状蓝图。
3. 按素材/复用键创建稳定媒体任务，预算按唯一生成素材计费。
4. 用 Sharp 合成分层预览，用 PptxGenJS 导出独立图片、文字和形状对象。
5. 扩展页面与整套审查输入，保留知识、素材、布局三类最小修复路径。
6. 通过合同、渲染、幂等、预算、E2E 和 V2 回归后接入 FrameFlow 功能开关。

检查点：Mock V3 PPTX 的底图、多图片、文字和形状均为独立对象；默认封面独立；`bun run check` 全绿。

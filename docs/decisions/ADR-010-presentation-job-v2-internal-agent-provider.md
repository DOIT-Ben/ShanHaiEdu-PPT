...
- 主 `server.ts` 只在配置独立 V2 服务 Token 后注入 `InternalPresentationJobV2Provider`。它把 V2 的冻结逐页设计映射为派生 tenant 下的私有 `VISUAL_DECK_V4` Run，固定 `BOUNDED_AUTO`、最多 4 轮返修和每页最多 5 次图片操作预算；未配置时保持 V1-only 运行。
- 派生 tenant 只用于内部记录和 Artifact 读取；外部 tenant、user、project 经过不可逆摘要映射。V2 响应不公开内部 Run ID、Step、Event、Delivery、提示词或模型诊断。
- 主进程 V2 路由只接受独立 `PPT_AGENT_V2_API_TOKEN`；V1 用户 Token、管理员 Token 和宿主回调 Token 均不能调用 V2。缺少该 Token 时不装配 V2 repository、Provider 或预算路由，V2 请求返回不可用。
...
- 主进程与 V2-only 入口都用进程内 `ticking` 门禁保证同一进程不重叠推进 V2 队列；当前 SQLite 数据根是单写进程边界。部署多个 V2 Worker 共享同一数据根前，必须先增加跨进程 lease 或 compare-and-swap，本决策不授权这种拓扑。

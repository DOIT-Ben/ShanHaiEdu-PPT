...
- V2 不再把只有 ZIP 魔数的任意字节发布为 PPTX。Artifact 在 CRC 解压前先限制 2,048 个 entry、256 MiB 单 entry、512 MiB 总未压缩量、200:1 压缩比，以及 4 MiB 单 XML part 和 16 MiB XML 总量；随后校验 OPC Content Types、presentation/slide MIME Override、根关系、页面关系、页数和每个 PresentationML `sld` 根元素。
